...
class _TutorProfileScreenState extends State<TutorProfileScreen> {
  ...
  late bool _isFavorite = student?.favorites.any((tutor) => tutor == widget.tutor.id) ?? false;
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: () {
                          ...
                          final currentFavorites = student!.favorites.map((t) => t).toList();
                          if (!currentFavorites.contains(widget.tutor.id)) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text("Gia sư này đã có trong danh sách yêu thích")),
                            );
                          }
                        },
...
Widget _buildInfoTab() {
  return Padding(
    ...
    child: Column(
      ...
      children: [
        _buildInfoRow(Icons.school, 'Môn dạy: ${widget.tutor.subjects.join(', ')}'),
        _buildInfoRow(Icons.location_on, 'Khu vực: ${widget.tutor.user.address}'),
        _buildInfoRow(Icons.attach_money, 'Giá dạy: ${NumberFormatter.formatCurrency(widget.tutor.pricePerHour!)}VND/h'),
        _buildInfoRow(Icons.calendar_today, 'Kinh nghiệm: ${widget.tutor.experienceYears} năm'),
        SizedBox(height: 16),
    ...
    children: [
      ...
      ...availability.map((a) => Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("• ${a.weekday}: ${a.startTime!.format(context)} - ${a.endTime!.format(context)}"),
          ],
        ),
      )),
    ],
